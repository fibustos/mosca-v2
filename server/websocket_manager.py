...
from core.environment_3d import Environment3D, FlyAgent3D
...
def _validate_protocol_config(value: object) -> dict[str, float | int | str]:
    ...
    try:
        ...
        raise ValueError("Protocol durations must be numeric seconds.") from error
...
class SimulationSession:
    ...
    def step(self) -> dict[str, Any]:
        ...
        self.fly.step(FORWARD_STEP, yaw_velocity, pitch_velocity)
...
async def _run_protocol(
    websocket: WebSocket, session: SimulationSession, config: dict[str, float | int | str]
) -> None:
    ...
    try:
        for trial in range(1, trials + 1):
            ...
            learning_index = session.brain.odor_association_strength("right") * 100.0
            ...
            if trial < trials:
                ...
                if not await _wait_protocol_duration(session, iti):
                    stopped = True
                    break
        else:
            completed = True
        if completed:
            try:
                session.brain.save_weights(_protocol_profile_path(profile_name))
